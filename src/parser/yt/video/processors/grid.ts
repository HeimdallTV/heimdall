 ...
 * todo: unused
 */
export function processGridVideo({
	gridVideoRenderer: video,
}: GridVideo): std.Video {
	const lengthText = findRenderer("thumbnailOverlayTimeStatus")(
		video.thumbnailOverlays,
	)?.text;
	if (!lengthText)
		throw Error("Grid video did not contain thumbnail overlay time status"); // TODO: Why is this here

	if (
		combineSomeText(video.title) === "NASA Live: Official Stream of NASA TV"
	) {
		console.log(video);
	}
	...
	return {
		...
		publishDate:
			...
			relativeToAbsoluteDate(combineSomeText(video.publishedTimeText)),
	};
}
