...
export const fetchRecommendedContinuation = (
	...
	AppendContinuationItemsResponse<
		RichItem<Video | Renderer<"radio">> | ContinuationItem
	>
...
export const fetchVideo = (videoId: string) =>
	fetchYt<VideoResponse>(Endpoint.Next, { videoId });
...
const getLikeButtonParams = async (
	...
	const paramName = std.matchLikeStatus(
		likeStatus,
		"likeParams",
		"removeLikeParams",
		"dislikeParams",
	...
	const {
		likeButtonViewModel: likeButton,
		dislikeButtonViewModel: dislikeButton,
	} =
		video.contents.twoColumnWatchNextResults.results.results.contents[0]
			.videoPrimaryInfoRenderer.videoActions.menuRenderer.topLevelButtons[0]
			.segmentedLikeDislikeButtonViewModel;
	const button = useLike
		? likeButton.likeButtonViewModel
		: dislikeButton.dislikeButtonViewModel;
	const commands =
		button.toggleButtonViewModel.toggleButtonViewModel[subButtonViewModel]
			.buttonViewModel.onTap.serialCommand.commands;
...
export async function fetchPlaybackTracking(videoId: string) {
	...
	const playbackURL = new URL(
		player.playbackTracking.videostatsPlaybackUrl.baseUrl.replace(
			"s.youtube.com",
			"www.youtube.com",
		),
	);
