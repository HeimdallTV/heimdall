...
export const processShelf = ({
	...
	HorizontalList<GridVideo> | HorizontalList<GridChannel>
>): std.Shelf => {
	...
	return {
		...
		type: isVideo ? std.ShelfType.Videos : std.ShelfType.Channels,
		name: combineSomeText(shelf.title),
		shortDescription: shelf.subtitle && combineSomeText(shelf.subtitle),
		href:
			shelf.navigationEndpoint &&
			getBrowseEndpointUrl(shelf.navigationEndpoint),
		items: shelf.content.horizontalListRenderer.items
			.map((renderer) => {
				if (isRenderer("gridVideo")(renderer))
					return processGridVideo(renderer);
				if (isRenderer("gridChannel")(renderer))
					return processGridChannel(renderer);
				console.warn(
					`Unknown renderer type "${
						Object.keys(renderer)[0]
					}" in shelf... ignoring`,
				);
			})
