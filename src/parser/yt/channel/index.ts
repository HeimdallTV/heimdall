...
export const getChannel = (channelId: string): Promise<std.Channel> =>
	processChannelPage(channelId);

export async function* listChannelVideos(
	channelId: string,
): AsyncGenerator<std.Video[]> {
	const channelVideosIterator = makeContinuationIterator(
		() =>
			// todo: move to separate parser
			fetchChannelVideos(channelId).then((res) => {
				const tabRenderer = unwrapRenderer(res.contents).tabs.find(
					isTab(ChannelTabName.Videos),
				);
				if (!tabRenderer)
					throw Error("Failed to find video tab in YT response");
				const richGridRenderer = unwrapRenderer(tabRenderer).content;
				if (!richGridRenderer)
					throw Error(
						"Failed to find list of videos in the video tab in YT response",
					);
		...
		(token) =>
			fetchChannelVideosContinuation(token).then(getContinuationResponseItems),
	);

	for await (const channelVideos of channelVideosIterator) {
		console.log("channelVideos", channelVideos);
		yield channelVideos
			.map((_) => _.richItemRenderer.content)
			.map(processVideo);
	}
}
...
export const listChannelLiveVideos = async (
	...
	fetchChannelLive(channelId)
		...
		.then(getTabContent)
		.then((live) =>
			live.richGridRenderer.contents
				.flatMap((grid) => grid.richItemRenderer.content)
				.map(processVideo),
		);

export const listChannelShelves = async (
	channelId: string,
): Promise<std.Shelf[]> =>
...
export const listChannelLinks = async (channelId: string): Promise<string[]> =>
	fetchChannelAbout(channelId).then(({ aboutChannelRenderer: about }) =>
		about.metadata.aboutChannelViewModel.links
			.map(
				(link) =>
					link.channelExternalLinkViewModel.link.commandRuns[0].onTap
						.innertubeCommand,
			)
			.map(getEndpointUrl)
