...
export async function setPlaylistVisibility(
	...
	const playlistPrivacy =
		visibility === std.Visibility.Public
			? 0
			: visibility === std.Visibility.Unlisted
			  ? 1
			  : 2;
	await sendAction(playlistId, Actions.SetPrivacy, { playlistPrivacy });
}
export async function setPlaylistTitle(
	playlistId: string,
	title: string,
): Promise<void> {
...
export async function movePlaylistVideo(
	...
	const movedSetVideoIdPredecessor =
		videos[newPosition - offset].playlistVideoRenderer.setVideoId;
