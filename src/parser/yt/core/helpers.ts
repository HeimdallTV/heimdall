...
export function fromShortHumanReadable(text: string): number {
  ...
  if (Number.isNaN(number)) throw Error(`Unable to parse number in short human readable number: "${text}"`)
...
export const extractNumber = (number: string) => {
  const numberExtracted = number.match(/[\d,]+/)
  if (numberExtracted === null) {
    throw new Error(`Unable to extract number from "${number}"`)
  }
  return Number(numberExtracted[0].replaceAll(',', ''))
}
