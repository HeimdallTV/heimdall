export const divideByAndConcat =
	(divisor: number, suffix: string) => (num: number) => {
		const divided = num / divisor;
		const stringified =
			divided >= 10 ? String(Math.floor(divided)) : divided.toFixed(1);
...
export function fromShortHumanReadable(text: string): number {
	...
	if (Number.isNaN(number))
		throw Error(
			`Unable to parse number in short human readable number: "${text}"`,
		);

	const suffix = relativeNumber.replaceAll(/[\d\.]/g, "").toLowerCase();
	if (suffix.length > 1) {
		throw new Error(
			`Detected more than one suffix "${suffix}" in short human readable number: "${text}"`,
		);
	}
	...
	throw new Error(
		`Unknown suffix "${suffix}" provided in short human readable number: "${text}"`,
	);
}
...
export const extractNumber = (viewCount: string) =>
	Number(
		viewCount
			.split(/([\d,]+)/)
			.filter(Boolean)[0]
