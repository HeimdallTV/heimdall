...
export type Source<Type extends SourceType = SourceType> = BaseSource<Type> &
	(Type extends SourceType.Audio | SourceType.AudioVideo
		? AudioSource
		: Record<never, never>) &
	(Type extends SourceType.Video | SourceType.AudioVideo
		? VideoSource
		: Record<never, never>);
...
export const isAudioSource = (
	source: Source,
): source is Source<SourceType.Audio> => source.type === SourceType.Audio;
export const isVideoSource = (
	source: Source,
): source is Source<SourceType.Video> => source.type === SourceType.Video;
export const isAudioVideoSource = (
	source: Source,
): source is Source<SourceType.AudioVideo> =>
...
export type ClosedCaptionTrack =
	| ClosedCaptionTrackCue[]
	| ClosedCaptionTrackCueLine[];
...
export type ClosedCaptionTrackCueWord = Omit<
	ClosedCaptionTrackCue,
	"endTimeMS"
>;
