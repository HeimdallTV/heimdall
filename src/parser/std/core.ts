...
export const matchLikeStatus = <
	ReturnType extends string | number | boolean | object,
>(
	...
	if (likeStatus === LikeStatus.Like)
		return typeof onLike === "function" ? onLike() : onLike;
	if (likeStatus === LikeStatus.Dislike)
		return typeof onDislike === "function" ? onDislike() : onDislike;
	if (likeStatus === LikeStatus.Indifferent) {
		return typeof onIndifferent === "function"
			? onIndifferent()
			: onIndifferent;
	}
...
export type Provider = {
	...
	listChannelRecommended?: (
		channelId: string,
	) => AsyncGenerator<(Video | Shelf)[]>;
