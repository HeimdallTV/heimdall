...
export const createPlayerInstance = (player: std.Player): PlayerInstance => {
	...
	const closedCaptions = createValueListener<std.ClosedCaption | undefined>(
		undefined,
	);
	...
	video.style.setProperty(
		"aspect-ratio",
		`${source.get().video.width} / ${source.get().video.height}`,
	);
	...
	const syncronizeIntervalId = setInterval(() => {
		...
		// Video is ahead
		if (video.currentTime - currentTime > 0.01)
			video.playbackRate = rate - synchronizationFactor;
		// Video is behind
	...
	video.addEventListener("durationchange", () => {
		const audioDuration = Number.isNaN(audio.duration)
			? undefined
			: audio.duration * 1000;
		const videoDuration = Number.isNaN(video.duration)
			? undefined
			: video.duration * 1000;
	...
	video.addEventListener("progress", () => {
		const bufferedRanges = new Array(video.buffered.length)
			...
			.map<[number, number]>((_, i) => [
				video.buffered.start(i) * 1000,
				video.buffered.end(i) * 1000,
			]);
...
export const usePlayerInstance = (player?: std.Player) => {
	const [instance, setInstance] = useState<PlayerInstance | undefined>(
		undefined,
	);
