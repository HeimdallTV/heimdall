import { ChannelShelf, PlaylistShelf, VideoShelf } from '@/components/Shelf'
    ...
    <div className="channel-max-width">
      {shelves.data.map((shelf, i) => {
        if (shelf.type === std.ShelfType.Videos) {
          return <VideoShelf key={shelf.name} {...shelf} size={i === 0 ? 'md' : 'sm'} videos={shelf.items} />
        }
        if (shelf.type === std.ShelfType.Channels) {
          return <ChannelShelf key={shelf.name} {...shelf} size="sm" channels={shelf.items} />
        }
        if (shelf.type === std.ShelfType.Playlists) {
          return <PlaylistShelf key={shelf.name} {...shelf} size="sm" playlists={shelf.items} />
        }
