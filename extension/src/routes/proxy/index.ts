        ...
        action: {
          ...
          requestHeaders: [
            ...
            },
            {
              header: 'Referer',
              operation: 'set',
              value: referer ?? new URL(url).origin,
            },
          ],
        },
        condition: {
...
export const fetch = async (
  ...
  const browserCookies = await browser.cookies
    ...
    .then((cookies) =>
      cookies
        .filter((cookie) => !cookie.expirationDate || cookie.expirationDate > Number(new Date()) / 1000)
        .map((cookie) => [cookie.name, cookie.value]),
    )
    .then(Object.fromEntries)
  const removeCookieRule = await createHeaderRule(
    url,
    { ...browserCookies, ...requestCookies },
    headers.get('origin'),
    headers.get('referer'),
  )
